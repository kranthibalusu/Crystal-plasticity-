...
610 FORMAT(A65536)
 ...
 do
   ...
   chunkNo=size(IO_stringPos(line))
   allocate(chunkPos(chunkNo))  
   ...
   IF (ALLOCATED (chunkPos)) DEALLOCATE (chunkPos)
...
! integer(pInt), dimension(5)           :: IO_stringPos

 character(len=*),                        intent(in) :: string                                      !< string in which chunk positions are searched for
 
 character(len=*), parameter  :: SEP=achar(44)//achar(32)//achar(9)//achar(10)//achar(13)           ! comma and whitespaces
 integer                      :: left, right, chunkNo                                                        ! no pInt (verify and scan return default integer)
 ...
 do while (verify(string(right+1:),SEP)>0)
   ...
   chunkNo = chunkNo+1_pInt
 enddo
  
   allocate(IO_stringPos(chunkNo*2+1))
 ...
 do while (verify(string(right+1:),SEP)>0)
   ...
   right = left + scan(string(left:),SEP) - 2
   if ( string(left:left) == '#' ) exit
   IO_stringPos(IO_stringPos(1)*2+2)=int(left, pInt)
   IO_stringPos(IO_stringPos(1)*2+3)=int(right, pInt)
