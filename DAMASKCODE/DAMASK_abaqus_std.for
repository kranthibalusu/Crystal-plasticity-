...
#define FEM
#define DAMASK_NUM_THREADS 1


#include "C:\Users\astl2\Desktop\CPFEMDAMASK\DAMASK\code\prec.f90"
...
subroutine DAMASK_interface_init
 ...
 write(6,'(/,a)')              ' Version: '// 'v2.0.1' !DAMASKVERSION
...
#include "C:\Users\astl2\Desktop\CPFEMDAMASK\DAMASK\code\compilation_info.f90"  
...
#include "C:\Users\astl2\Desktop\CPFEMDAMASK\DAMASK\code\commercialFEM_fileList.f90"
...
!     ABAQUS implicit:     11, 22, 33, 12
 ...
 if(symmetricSolver) then
     ddsdde(1:ntens,1:ntens) = 0.5_pReal*(ddsdde(1:ntens,1:ntens) + transpose(ddsdde(1:ntens,1:ntens)))
 end if
 if(ntens == 6) then
   stress_h = stress
   stress(5) = stress_h(6)
 ...
 !if (output) then
	 !write(6,*) 'theTime2',theTime	
	 !if(noel == 1) then 
		! write(6,*) 'state'
	! write(*,*) crystallite_postResults(1,npt,mesh_FEasCP('elem', noel))

	 !endif
 !endif 

 !write(*,*) 'postsize', shape(crystallite_postResults(1,npt,mesh_FEasCP('elem', noel)))
 !statev = materialpoint_results(1:min(nstatv,materialpoint_sizeResults),npt,mesh_FEasCP('elem', noel))

 statev = crystallite_postResults(1,npt,mesh_FEasCP('elem', noel))
!write(*,*) 'sadsaf1'
  

 if ( terminallyIll ) then
     pnewdt = 0.5_pReal                                                            ! force cutback directly ?
