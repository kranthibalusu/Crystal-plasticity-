...
#include <petsc/finclude/petsc.h90>
 ...
 integer, protected, public :: &
   DAMASK_NumThreadsInt       =  1                                                                  !< value stored in environment variable DAMASK_NUM_THREADS, set to zero if no OpenMP directive
 integer(pInt), public :: &
...
!$ integer ::                                gotDAMASK_NUM_THREADS = 1
 ...
 integer(pInt), dimension(5) :: chunkPos
 character(len=65536) :: &
   tag ,&
   line
...
! read variables from config file and overwrite default parameters if keyword is present
   ...
   do while (trim(line) /= IO_EOF)                                                                 ! read thru sections of phase part
	...
	! skip empty lines
	! allocate(chunkPos(size(IO_stringPos(line))))
     chunkPos = IO_stringPos(line)
	! write(*,*) chunkPos


     tag = IO_lc(IO_stringValue(line,chunkPos,1_pInt))   
	! write(*,*) trim(tag)
